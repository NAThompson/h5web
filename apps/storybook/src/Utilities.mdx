...
- `errors` – the errors to be added/subtracted to the values (one error per value)
- `includeErrors` – whether to return the domain with or without the errors (`true` by default)
- `scaleType` – the scale with which the returned domain must be compatible:
  - with `ScaleType.Log`, the min is clamped to the first strictly positive value
  - with `ScaleType.Sqrt` the min is clamped to the first positive value including 0
- `ignoreValue` – a function called for each value in the array; if it returns `true`, the value is ignored from the domain computation

Note that `getDomain` returns `undefined` if:

- all the values in the array are infinite, NaN or ignored via `ignoreValue`;
- the domain is not compatible with the given `scaleType` (e.g. `[-x, 0]` in log scale).

```ts
getDomain(values: AnyNumArray, opts?: GetDomainOpts & { errors?: AnyNumArray }): Domain | undefined

const linearDomain = getDomain([10, 5, -1]); // [-1, 10]
const logDomain = getDomain([-1, 0, 2], { scaleType: ScaleType.Log }); // [2, 2]
const sqrtDomain = getDomain([-1, 0, 2], { scaleType: ScaleType.Sqrt }); // [0, 2]

const myArray = ndarray([0, 1, 2, 3], [2, 2]);
const myDomain = getDomain(myArray); // [0, 3]

const withErrors = getDomain([2, -1, 10], { errors: [1, 2, 3] }); // [-3, 13]
const withoutErrors = getDomain([2, -1, 10], { errors: [1, 2, 3], includeErrors: false }); // [-1, 10]
const withIgnore = getDomain([30, 20, 10], { ignoreValue: (val) => val > 20 }); // [10, 20]

const undef1 = getDomain([2, NaN, Infinity], { ignoreValue: (val) => val === 2 }); // undefined
const undef2 = getDomain([0, -1, -2], { scaleType: ScaleType.Log }); // undefined
```
...
Called internally by `VisCanvas` to compute the space needed around the visualization for the title, axes labels, etc.
Useful for implementing advanced layouts, like keeping the axes flush with the visualization in an equal-aspect canvas.

```ts
function getAxisOffsets(hasLabel?: {
  left?: boolean; // whether left axis has label
  bottom?: boolean; // whether bottom axis has label
  top?: boolean; // whether visualization has title
}): AxisOffsets;
```
...
Slicing or transposing an ndarray with `NdArray#pick()` or `NdArray#transpose()` creates a "view" of the original ndarray but with the same underlying `data` array.
This means that the order of the values in the internal `data` array of an ndarray view does not necessarily reflect the shape of the view.
This is a problem for some of H5Web's WebGL visualization, which rely on this internal `data` array, like `HeatmapVis`.

`createArrayFromView` copies an ndarray view into a new ndarray with an internal `data` array that matches its shape.

```ts
function createArrayFromView<T extends ArrayValue>(
  view: NdArray<T>,
): NdArray<T>;

const sourceArr = ndarray([0, 1, 2, 3], [2, 2]);
const transposedView = sourceArr.transpose(1, 0); // transposedView.data: [0, 1, 2, 3]
const transposedArr = createArrayFromView(view); // transposedArr.data: [0, 2, 1, 3]
```
...
Keep a geometry instance up to date. Requires passing an instance of a Three.js `BufferGeometry` subclass
that implements the `H5WebGeometry` interface.
...
const lineGeometry = useMemo(
  () => new LineGeometry({abscissas, ordinates, abscissaScale, ordinateScale }),
  [abscissas, ordinates, abscissaScale, ordinateScale]
);

useUpdateGeometry(lineGeometry, { skipUpdates: !visible });
```
...
- `LineConstantGeometry`, used in [`Line`](https://h5web-docs.panosc.eu/?path=/docs/building-blocks-line--docs) with constant interpolation
